...
namespace SikuliSharp {
	public class Region : IRegion {
		private static readonly Regex RegionRegex = new Regex(@"R\[(\d*),\s{0,}(\d*) (\d*)x(\d*)\]", RegexOptions.Compiled);
		...
		public Region(string regionString) {
			var regionMatchs = RegionRegex.Match(regionString);
			_x = int.Parse(regionMatchs.Groups[1].ToString());
			_y = int.Parse(regionMatchs.Groups[2].ToString());
			_w = int.Parse(regionMatchs.Groups[3].ToString());
			_h = int.Parse(regionMatchs.Groups[4].ToString());
		}
